...
// NOLINTNEXTLINE
TEST_F(evds_lambda, add_handler)
{
    EXPECT_TRUE(e->add_handler<void>("", [](){}));
}
...
TEST_F(evds_lambda, remove_event_handler_after_add)
{
    ...
    EXPECT_EQ(id, 1);
    EXPECT_TRUE(e->remove_handler(1));
    EXPECT_FALSE(e->remove_handler(1));
}
...
TEST_F(evds_lambda, mix_add_remove_event_handlers)
{
    ...
    auto id3 = e->add_handler("2", [](){}); // already added
    EXPECT_EQ(id3, 3);

    EXPECT_TRUE(e->remove_handler(id1));
    EXPECT_TRUE(e->remove_handler(id2));
    EXPECT_FALSE(e->remove_handler(id2)); // already removed
    EXPECT_FALSE(e->remove_handler(id3)); // never added
}
...
TEST_F(evds_lambda, push_event_n_times_void_payload)
{
    ...
    int count = -1;
    e->add_handler(event_name, [&count, &cv]
    {
        count++;
        cv.notify_one();
    });
...
TEST_F(evds_lambda, push_event_n_times_increasing_payload)
{
    ...
    const auto event_name = "EVENT_NAME";
    int count = -1;
    e->add_handler(event_name, [&count, &cv](int expected_count_arg)
    {
        count++;
...
TEST_F(evds_lambda, push_event_n_times_same_payload)
{
    ...
    int count = -1;
    e->add_handler(event_name, [&expected_value, &count, &cv](int expected_value_arg)
    {
        count++;
...
{
    const auto event_name = "EVENT_NAME";
    EXPECT_TRUE(e->add_handler(event_name, [](int i){}));
    EXPECT_TRUE(e->add_handler(event_name, [](unsigned int i){}));
    EXPECT_TRUE(e->add_handler(event_name, [](unsigned long i){}));
    EXPECT_TRUE(e->add_handler(event_name, [](long long i){}));

    EXPECT_TRUE(e->add_handler(event_name, [](std::string s){}));
    EXPECT_TRUE(e->add_handler(event_name, [](std::string& s){}));
    EXPECT_TRUE(e->add_handler(event_name, [](std::string&& s){}));
    EXPECT_TRUE(e->add_handler(event_name, [](const std::string& s){}));
    
    EXPECT_FALSE(e->add_handler(event_name, [](const std::string s){})); // same as non-const, non-ref
}
...
TEST_F(evds_lambda, push_multiple_handlers_same_event_name)
{
    ...
    auto int_payload = 0;
    EXPECT_TRUE(e->add_handler(event_name, [&cv, &int_payload](int i)
    {
        int_payload = i;
        cv.notify_one();
    }));
    ...
    EXPECT_TRUE(e->add_handler(event_name, [&cv, &str_payload](std::string s)
    {
        str_payload = s;
        cv.notify_one();
    }));
...
TEST_F(evds_lambda, push_event_consecutive_start_stop)
{
    ...
    const auto event_name = "EVENT_NAME";
    int payload_value = 0;
    e->add_handler(event_name, [&payload_value, &cv](int expected_value_arg)
    {
...
TEST_F(evds_lambda, push_event_with_different_overloads)
{
    ...
    EXPECT_TRUE(e->add_handler(event_name, [](float f1, float f2, float f3){}));
    ...
    EXPECT_TRUE(e->add_handler(event_name, [](int i1, int i2, int i3){}));
...
{
    const auto event_name = "EVENT_NAME";
    EXPECT_TRUE(e->add_handler(event_name, [](int i, std::string s){}));
    EXPECT_FALSE((e->remove_handler<void, int>(event_name))); // no match void(int)
    EXPECT_TRUE((e->remove_handler<void, int, std::string>(event_name))); // match: void(int, string)
    EXPECT_FALSE((e->remove_handler<void, int, std::string>(event_name))); // already removed (previous line)
}
